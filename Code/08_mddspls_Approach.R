...
eval_mddspls_approach <- function(path = '/BWM-Article/Data/Raw/BLCA.Rda', frac_train = 0.75, split_seed = 1312,
...
for (curr_train_pattern in seq(from = args[2], to = 5, by = 1)) {
  ...
  for (curr_test_pattern in seq(from = start_value_test, to = 4, by = 1)) {
    ...
    for (curr_repetition in seq(from = start_value_rep, to = 5, by = 1)) {
      ...
      write.csv(mddspls_res, paste0('/BWM-Article/Docs/Evaluation_Results/MDDSPLS_Approach/MDDSPLS_Eval_', args[1], '_', args[2], '_', args[3], '_', args[4], '.csv'))
      rm(curr_res)
      gc()
    }
